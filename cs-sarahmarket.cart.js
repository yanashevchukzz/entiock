...
function addToCartSuccess (jqXHR, textStatus, errorThrown){
  ...
  var ai_numpro = ""; if ($(".num-items-in-cart span.number").html() ==1) ai_numpro = "Hay 1 producto en su carrito."; else ai_numpro = "Hay"+$(".num-items-in-cart span.number").html()+"productos en su carrito.";
...
function addToCartFail(jqXHR, textStatus, errorThrown){
  ...
  var $info = '<span class="cirle"><i class="fa fa-times"></i></span><div class="error">Agregar a carrito fallo!<br/>Error:'+ response.description +'</div>';
  var ai_numpro = ""; if ($(".num-items-in-cart span.number").html() ==1) ai_numpro = "Hay 1 producto en su carrito."; else ai_numpro = "Hay"+$(".num-items-in-cart span.number").html()+"productos en su carrito.";
...
Shopify.updateCartInfo = function(cart, cart_summary_id, cart_count_id) {
          ...
          if (key === 'items') {
            ...
            if (value.length) {
              ...
              jQuery.each(value, function(i, item) {
                ...
                jQuery('<div class="action"><button class="btn" onclick="window.location=\'/cart\'">Carrito<i class="fa fa-caret-right"></i></button><button class="btn float-right" onclick="window.location=\'/checkout\'">Pagar<i class="fa fa-caret-right"></i></button></div>').appendTo(cart_summary); 
            
            }
            else {
              jQuery('<div class="empty text-center"><em>Tu carrito esta vacio. <a href="/collections/all" class="btn">Seguir comprando</a></em></div>').appendTo(cart_summary);
            }
          }
        });
      }
    ...
    if ((typeof cart_count_id) === 'string') {
      if (cart.item_count == 0) { 
        jQuery('#' + cart_count_id).html('Su carrito esta vacio');
      }
      else if (cart.item_count == 1) {
        jQuery('#' + cart_count_id).html('1 producto en su carrito');
      }
        else {
          jQuery('#' + cart_count_id).html(cart.item_count + 'productos en su carrito');
        }
    }
  ...
  function updateCartDesc(data){
    ...
    var $cartLinkText_1 = $('.items-cart-group .cart_text');
    //var $cartPrice = '<span class="total"> - '+ Shopify.formatMoney(data.total_price, cart_money_format) +'</span>';
    switch(data.item_count){
      case 0:
        $cartLinkText.html('<span class="number">0</span>');
        $cartLinkText_1.html('<span class="number">0</span>');
        break;
      case 1:
        $cartLinkText.html('<span class="number">1</span>');
        $cartLinkText_1.html('<span class="number">1</span>');
        break;
      default:
        $cartLinkText.html('<span class="number">'+data.item_count+'</span>');
        $cartLinkText_1.html('<span class="number">'+data.item_count+'</span>');
        break;
    }
